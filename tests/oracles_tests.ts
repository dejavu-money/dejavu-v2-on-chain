...
describe("Oracles methods", () => {
  ...
  describe("#join-oracle", async () => {
    it("joins players to an oracle hub", async () => {
      const organizationId = new Date().getTime();
      const oracleId = new Date().getTime();
      const startAtUtcUnix = new Date().getTime();
      ...
      assert.equal(bets[0].globalBetsIndex, 0, "verify bet index");

      assert.equal(bets[0].gameResult, 0, "verify bet game result");

      assert.equal(playerBetData.index, 0, "verify player bet index");
  ...
  describe("#withdraw-from-oracle", async () => {
    it("withdraws from oracle", async () => {
      const organizationId = new Date().getTime();
      const oracleId = new Date().getTime();
      const startAtUtcUnix = new Date().getTime();
